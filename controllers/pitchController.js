...
const createPitch = (req, res, next) => {
  const { name, location, date, startTime, description, capacity, type, imageUrl } = req.body;

  FootballPitch.create({ name, location, date, startTime, description, capacity, type, imageUrl })
    .then((newPitch) => {
